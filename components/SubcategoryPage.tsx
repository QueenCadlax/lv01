...
const getCategoryDetectionFlags = (subCategoryName: string, categoryKey?: Category) => {
  ...
  const isSecurity = categoryKey === Category.SecurityProtectionAndResponse || /SECURITY|PROTECTION|ALARM|SURVEILLANCE|GUARD|EMERGENCY|RESPONSE/i.test(subCategoryName);
  ...
  const isFamily = /FAMILY|CHILDCARE|COMMUNITY|CHILDREN|KIDS|CHURCH|MOSQUE|PLAYCENTRE|PLAY CENTRE|AFTERSCHOOL|RELIGIOUS/i.test(subCategoryName);
  ...
  // Consolidated flag for Home / Construction / Trades -> SERVICES
  const isServices = categoryKey === Category.HomeConstructionAndTrades || /SERVICES/i.test(subCategoryName) || isBuilders || isPlumbingElectrical || isRoofingRenovations;
  ...
  isNailBeauty, isCosmeticSurgery, isNutritionists, isSkincare, isSpa, isHealth, isSecurity, isEducation, isFamily, isRealEstate,
...
const SubcategoryPage: React.FC<Props> = ({ categoryName, subCategoryName, listings, heroImage, navigate, selectedId, onSelectSubcategory }) => {
  ...
  // If this is the Education category (or any education subcategory), render the premium education component
  if (categoryKey === Category.EducationAndSkills) {
    return <EducationPremium navigate={navigate || (() => {})} businesses={listings} />;
  }

  // If this is Home / Construction & Trades, render the Services page
  if (categoryKey === Category.HomeConstructionAndTrades) {
    return <ServicesPage navigate={navigate || (() => {})} businesses={listings} categoryKey={categoryKey} />;
  }

  // If this is Security / Protection & Response, render the Services page (use services template)
  if (categoryKey === Category.SecurityProtectionAndResponse) {
    return <ServicesPage navigate={navigate || (() => {})} businesses={listings} categoryKey={categoryKey} />;
  }

  // If this is Pets / Veterinary & Animal Care, render the Services page with pet-specific copy
  if (categoryKey === Category.PetsVeterinaryAndAnimalCare) {
    return <ServicesPage navigate={navigate || (() => {})} businesses={listings} categoryKey={categoryKey} />;
  }

  // If this is Sports / Fitness & Recreation, render the Services page with sports-specific copy
  if (categoryKey === Category.SportsFitnessAndRecreation) {
    return <ServicesPage navigate={navigate || (() => {})} businesses={listings} categoryKey={categoryKey} />;
  }

  // If this is Events / Experiences & Occasions, render the Services page with events-specific copy
  if (categoryKey === Category.EventsExperiencesAndOccasions) {
    return <ServicesPage navigate={navigate || (() => {})} businesses={listings} categoryKey={categoryKey} />;
  }

  // If this is Digital, Media & Technology, render the Services page with digital-specific copy
  if (categoryKey === Category.DigitalMediaAndTechnology) {
    return <ServicesPage navigate={navigate || (() => {})} businesses={listings} categoryKey={categoryKey} />;
  }

  // If this is Shopping & Retail, render the Services page with shopping-specific copy
  if (categoryKey === Category.ShoppingAndRetail) {
    return <ServicesPage navigate={navigate || (() => {})} businesses={listings} categoryKey={categoryKey} />;
  }

  // If this is Transport & Auto Services, render the Services page with transport-specific copy
  if (categoryKey === Category.TransportChauffeursFleet) {
    return <ServicesPage navigate={navigate || (() => {})} businesses={listings} categoryKey={categoryKey} />;
  }
  ...
  : isSecurity
  ? 'Trusted security providers, surveillance specialists and emergency response services across Mpumalanga.'
    : isEducation
  ...
  // Decide whether to render the Healthcare homepage layout inline inside this
  // directory page. We do not return early any more; instead we hide the
  // SubcategoryPage hero and render `HealthPage` above the listings when this
  // flag is true.
  const _subUpper = (subCategoryName || '').trim().toUpperCase();
  const HEALTH_SUBS = ['GENERAL PRACTITIONER', 'GENERAL PRACTITIONERS', 'DENTIST', 'DENTISTS', 'GYNAECOLOGIST', 'GYNAECOLOGISTS', 'GYNAECOLOGY'];
  const renderHealthInline = Boolean(isHealth && (_subUpper === '' || HEALTH_SUBS.includes(_subUpper)));
  ...
  return (
    <div className="bg-black min-h-screen text-white">
      {renderHealthInline ? (
        // Render the Healthcare homepage layout inline so header/footer remain intact
        <div className="pb-8">
          <HealthPage navigate={(view: string) => navigate ? navigate(view, categoryName) : undefined} />
        </div>
      ) : (
        <>
              ...
              <div className="max-w-5xl mx-auto">
                ...
                <h1 className="text-5xl md:text-6xl lg:text-7xl font-serif text-white uppercase tracking-tight drop-shadow-lg mb-4">
                  {!hasSelectedSubcategory
                    ? (categoryKey === Category.SecurityProtectionAndResponse ? 'Security & Emergency' : categoryName || 'Experiences')
                    : subCategoryName}
                </h1>
                {/* Luxury Divider */}
                ...
                <p className="text-lg md:text-xl text-gray-200 font-light mb-4 max-w-2xl">
                  {heroTagline}
                </p>

                {/* Trust badges */}
                <div className="flex flex-wrap gap-3 mb-8">
                  <div className="px-3 py-1 bg-white/10 text-sm rounded-full text-white flex items-center gap-2">✓ <span className="font-medium">Verified Businesses</span></div>
                  <div className="px-3 py-1 bg-white/10 text-sm rounded-full text-white flex items-center gap-2">✓ <span className="font-medium">Trusted Professionals</span></div>
                  <div className="px-3 py-1 bg-white/10 text-sm rounded-full text-white flex items-center gap-2">✓ <span className="font-medium">Premium Listings</span></div>
                  <div className="px-3 py-1 bg-white/10 text-sm rounded-full text-white flex items-center gap-2">✓ <span className="font-medium">Updated Regularly</span></div>
                </div>
              </div>
            </div>
          </section>
          ...
          <div className="container mx-auto px-4 -mt-4 relative z-20 mb-12">
            ...
            <div className="mb-6 max-w-4xl mx-auto">
              <div className="flex gap-2 flex-wrap">
                {['Open Now','Verified','Top Rated','Premium','Nearby','Most Reviewed'].map((chip) => (
                  <button key={chip} className="px-3 py-1 bg-white/10 text-sm rounded-full text-white hover:bg-white/20 transition">{chip}</button>
                ))}
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 flex-wrap mb-6 max-w-4xl mx-auto">
              ...
              {/* Subcategory filter - label changed */}
              ...
              {hasSelectedSubcategory === false && (
                ...
                {/* Sort By select (keeps same functionality - placeholder) */}
                <select
                  value={''}
                  onChange={() => {}}
                  className="px-4 py-2 bg-black border border-white/30 text-white rounded-full text-sm font-medium hover:border-white/50 transition outline-none cursor-pointer"
          ...
          <div className="mb-8">
            ...
            <div className="mb-4">
              ...
              <p className="text-sm md:text-base text-gray-400 max-w-2xl">
                Hand-selected businesses trusted by the LowveldHub community.
              </p>
            </div>
